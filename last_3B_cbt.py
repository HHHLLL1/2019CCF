# -*- coding: utf-8 -*-
"""
Created on Sat Nov 23 00:26:49 2019
...
select = SelectPercentile(percentile=percent)  # 选择特征重要度排在前百分之35的特征,默认计算函数是f_classif(只适用于分类问题)
...
for i in attr_feat:
    data1, predict_label = get_predict_w(lgb_attr_model, data1, label=i,
                                        feature=features, random_state=2019, n_splits=10, model_type='lgb')
...
print(pred_attr_feat)    
    
'''
划分数据，
丢弃之前的所有特征，
以预测出的B类4-6特征作为最后的特征进行结果预测 
'''
print(X_train.shape, X_test.shape)
tr_index = ~data1['label'].isnull()
X_train = data1[tr_index][pred_attr_feat].reset_index(drop=True)
y = data1[tr_index]['label'].reset_index(drop=True).astype(int)
X_test = data1[~tr_index][pred_attr_feat].reset_index(drop=True)
...
#sub.to_csv('./last/last_3B_cbt.csv', index=False)
